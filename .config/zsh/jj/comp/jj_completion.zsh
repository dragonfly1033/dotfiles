...
&& ret=0
    ...
    (bench)
        words=($line[1] "${words[@]}")
        (( CURRENT += 1 ))
        curcontext="${curcontext%:*:*}:jj-bench-command-$line[1]:"
        case $line[1] in
            (common-ancestors)
_arguments "${_arguments_options[@]}" : \
'-s+[Name of baseline to save results]:SAVE_BASELINE:_default' \
'--save-baseline=[Name of baseline to save results]:SAVE_BASELINE:_default' \
'-b+[Name of baseline to compare with]:BASELINE:_default' \
'--baseline=[Name of baseline to compare with]:BASELINE:_default' \
'--sample-size=[Sample size for the benchmarks, which must be at least 10]:SAMPLE_SIZE:_default' \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
'--repository=[Path to repository to operate on]:REPOSITORY:_files -/' \
'--at-operation=[Operation to load the repo at]:AT_OPERATION:_default' \
'--at-op=[Operation to load the repo at]:AT_OPERATION:_default' \
...
(revset)
_arguments "${_arguments_options[@]}" : \
'-f+[Read revsets from file]:FILE:_files' \
'--file=[Read revsets from file]:FILE:_files' \
'-s+[Name of baseline to save results]:SAVE_BASELINE:_default' \
'--save-baseline=[Name of baseline to save results]:SAVE_BASELINE:_default' \
'-b+[Name of baseline to compare with]:BASELINE:_default' \
'--baseline=[Name of baseline to compare with]:BASELINE:_default' \
...
'bench:Commands for benchmarking internal operations' \
...
(( $+functions[_jj__bench_commands] )) ||
_jj__bench_commands() {
    local commands; commands=(
'common-ancestors:Find the common ancestor(s) of a set of commits' \
'is-ancestor:Checks if the first commit is an ancestor of the second commit' \
'resolve-prefix:Resolve a commit ID prefix' \
'revset:Walk the revisions in the revset' \
    )
    _describe -t commands 'jj bench commands' commands "$@"
}
(( $+functions[_jj__bench__common-ancestors_commands] )) ||
_jj__bench__common-ancestors_commands() {
    local commands; commands=()
    _describe -t commands 'jj bench common-ancestors commands' commands "$@"
}
(( $+functions[_jj__bench__is-ancestor_commands] )) ||
_jj__bench__is-ancestor_commands() {
    local commands; commands=()
    _describe -t commands 'jj bench is-ancestor commands' commands "$@"
}
(( $+functions[_jj__bench__resolve-prefix_commands] )) ||
_jj__bench__resolve-prefix_commands() {
    local commands; commands=()
    _describe -t commands 'jj bench resolve-prefix commands' commands "$@"
}
(( $+functions[_jj__bench__revset_commands] )) ||
_jj__bench__revset_commands() {
    local commands; commands=()
    _describe -t commands 'jj bench revset commands' commands "$@"
}
